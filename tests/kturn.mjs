...
// What KTURN has to be for a keyboard player to be in the same game as a mouse
// one. The aim profile follows the real constant here rather than a copy of it,
// or the sweep would be measuring a number the game does not use.
...
console.log('');
console.log('=== HOW FAR A KEYBOARD PLAYER GETS, BY TURN RATE =================');
console.log('');
console.log('  Taking horn cards first, which is the strongest plan there is, so');
console.log('  this is turning speed and nothing else.');
console.log('');
console.log('  ' + P('KTURN', 9) + R('180 turn', 10) + R('worst', 8) + R('median', 8) +
            R('best', 7) + R('kills', 8));
const runAt = (rate) => {
  C._KTURN = rate;
  AIMS.keyboard = rate;
  const rs = SEEDS.map((sd) => run('all guns', 'keyboard', sd));
  const w = rs.map((r) => r.waves);
  console.log('  ' + P(rate, 9) + R((Math.PI / rate).toFixed(2) + 's', 10) +
    R(Math.min(...w), 8) + R(med(w), 8) + R(Math.max(...w), 7) +
    R(med(rs.map((r) => r.kills)), 8));
};
for (const r of [2.04, 3.5, 5, 6.5, 8]) runAt(r);
C._KTURN = 2.04;
const ms = SEEDS.map((sd) => run('all guns', 'mouse', sd));
const mw = ms.map((r) => r.waves);
console.log('');
console.log('  ' + P('mouse', 9) + R('~0.40s', 10) + R(Math.min(...mw), 8) +
  R(med(mw), 8) + R(Math.max(...mw), 7) + R(med(ms.map((r) => r.kills)), 8) +
  '   for comparison, unchanged');
